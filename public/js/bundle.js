(() => {
  ...
  var signup = async (name, email, password, passwordConfirm) => {
    try {
      ...
      if (res.data.status === "success") {
        showAlert(
          "success",
          res.status === 201 ? "Account created successfully!" : "Welcome back! You already have an account \u2014 logged you in."
        );
        window.setTimeout(() => {
          location.assign("/");
        }, 1500);
  ...
  // public/js/paystack.js
  var paystack_default = async (tourId) => {
    try {
